...
void Spawner::spawnEnemies(std::vector<Enemy>& enemigos, sf::Vector2f playerPosition, float aumentoVida, float aumentoDanio, float aumentoVelocidad) {
		...
		relojGeneracionEnemigos.getElapsedTime().asSeconds() >= ENEMY_INTERVAL) // Verifica si el tiempo transcurrido es mayor o igual al intervalo de generación
    {
        ...
        //Se le agrego aumentoDanio y aumentoVelocidad para que los enemigos puedan escalar por tiempo
		enemigos.emplace_back(50.0f + aumentoVida, 40.0f + aumentoVelocidad, 10.0f + aumentoDanio, "assets/fantasma.png", sf::Vector2f(x, y)); // Crear un nuevo enemigo en una posición aleatoria alrededor del jugador
